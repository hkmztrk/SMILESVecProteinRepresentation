...
import numpy as np
from lingoextract import *
from lingosim import *
...
def chooseProtSMIVector(bioid, bioid2, LINGOembds, maccsDict, protligdic, q, wordOrChar, opt):
    ...
    if opt == "1":
        ...
        if bioid not in proteinVectors.keys():
            biotxt1 = getProtSMIVector(LINGOembds, protligdic,  bioid, q, wordOrChar)
        else:
            biotxt1= proteinVectors[bioid]

        if bioid2 not in proteinVectors.keys():
            biotxt2 = getProtSMIVector(LINGOembds, protligdic,  bioid2, q, wordOrChar)
        else:
...
def constructSimMatrixv2(embFile, pairfile, wordOrChar, protOrLig, q, ifile, opt):   # text file folder & pair list
    ...
    maccsDict = json.load(open(maccspath))
    ...
    with open(pairfile) as fr:
        for line in fr:
            ...
            simsst = chooseProtSMIVector(bioid, bioid2, trainedEmbds, maccsDict, protligdic,  q, wordOrChar, opt)
