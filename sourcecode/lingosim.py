...
def loadEmbeddingsv2(LRNPATH):
    pcakey=50
    vectors = []
    keys = []
    embeddings_index = {}

    with open(LRNPATH) as f:
        for line in f:
            ...
            vectors.append(vec)
            #vectors[char] = vec
    pca = PCA(n_components=pcakey)
    vsize = pcakey
    X_pca = pca.fit_transform(np.array(vectors))

    for i, skeys in enumerate(keys):
    	embeddings_index[skeys] = X_pca[i]
